...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="vjobs",
        description="Search Vjobs listings from your terminal.",
    )
    ...
    parser.add_argument("--api", default=DEFAULT_API, help=f"Vjobs API base URL (default: {DEFAULT_API})")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Error contacting Vjobs API at {args.api}: {exc}", file=sys.stderr)
        return 1
    print_jobs(jobs)
    return 0
